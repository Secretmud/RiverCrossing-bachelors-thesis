...
def neighbor(surface, x, y):
    neigh = {0: surface[y + 1][x] if y + 1 < m else None,                       # North
             1: surface[y + 1][x + 1] if y + 1 < m and x + 1 < n else None,     # North East
             2: surface[y][x + 1] if x + 1 < n else None,                       # East
             3: surface[y - 1][x + 1] if y > 0 and x > 0 else None,             # South East
    ...
    for y in range(m):
        for x in range(n):
            if surface[y][x].visited == True:
                ...
                for ant in ants:
                    ant_path = ant.get()
                    dist = np.sqrt((ant.last_dy - a_start_y)**2)
                    if dist <= 4:
